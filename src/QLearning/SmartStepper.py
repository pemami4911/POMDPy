...
class SmartStepper(Sg.StepGenerator):
    """
    This class implements the methods extend_and_backup and get_step
    """
    def __init__(self, model, solver):
        self.model = model
        self.solver = solver
        self.logger = logging.getLogger('Model.SmartStepper')
    ...
    def extend_and_backup(self, seq, maximum_depth):
        ...
        if self.status is Sg.SearchStatus.CLEAN_FINISH or self.status is Sg.SearchStatus.TERMINATED:
            ...
            self.solver.update_sequence(entries)
